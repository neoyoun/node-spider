...
method.readFilePromise(dataPath)
	...
	.then(catesWithImg=>{
		...
		return Promise.all(imgsCollection.map(img=>{
			return downLoadImg(img.src, img.dir, img.filename)
		}))
	})
	/*.then(imgsCollection=>{
		return Promise.all(imgsCollection.map(img=>{
	...
	.then(allDownLoad=>{
		console.log(allDownLoad.length)
		console.log(allDownLoad[0])
	})
	.catch(err=>{
...
function downLoadImg(src, dir, filename) {
	return new Promise((resolve,reject)=>{
		...
		request.head(src, (err, res, body)=>{
			if(err){
				console.log(('request occur error'.filename).red)
				reject(err)
			} else {
